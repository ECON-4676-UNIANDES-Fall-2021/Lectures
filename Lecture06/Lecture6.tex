...
\medskip
 ...
 \medskip
   \begin{itemize}
     \item  $f(x|\theta) : \mathbb{R}\rightarrow  \mathbb{R}$ and $f(x,\theta)=\frac{1}{\sqrt{2\pi\sigma}}exp\left(- \frac{(x-\mu)^2}{2\sigma^2} \right)$
     \medskip
     \item  $ \mathcal{L}(\theta|x) : \mathbb{R}^2 \rightarrow  \mathbb{R}$ and  $\mathcal{L}(\theta|x) =\frac{1}{\sqrt{2\pi\sigma}}exp\left(- \frac{(x-\mu)^2}{2\sigma^2} \right)$
   \end{itemize}
\medskip
   \item Intuition?
\end{itemize}
...
\mathcal{L}(\theta|x)=\Pi_{i=1}^n \mathcal{L}(\theta|x)=\Pi_{i=1}^n f(x_i|\theta)
\end{align}

A maximum likelihood estimator of the parameter $\theta$:

\begin{align}
\hat \theta^{MLE}=\underset{\theta \in \Theta}{argmax}\, \mathcal{L}(\theta|x)(\theta,x)
\end{align}
