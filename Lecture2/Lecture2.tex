...
\begin{itemize}
  \item Classical Stats (small data?)
    \begin{itemize}
      ...
      \item Lost of structure, e.g. $X_1,X_2,...,X_n\sim t_v$
...
\begin{itemize}
  ...
  \medskip
  \item On the other hand, if we prefer to think about approximating $E(Y|X)$, as opposed to predicting $Y$, the Regression-CEF theorem tells us that even if the CEF is nonlinear, regression provides the best linear approximation to it.
\medskip
  \item The statement that regression approximates the CEF lines up with our view of empirical work as an effort to describe the essential features of statistical relationships, without necessarily trying to pin them down exactly. 
  \medskip
